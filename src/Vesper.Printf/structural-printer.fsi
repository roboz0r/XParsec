...
/// <summary>The concrete <see cref="Vesper.IFormatSink"/>: records a value's declared
/// structure into a layout document, then lays that document out to a string within a
/// column + node budget. The layout state itself is not part of the contract.</summary>
type RuntimeFormatState =
    ...
    /// <paramref name="printSize"/> (F# PrintSize; nodes past it render <c>...</c>).</summary>
...
/// <summary>Entry point for <c>%A</c>: renders a value as copy-pasteable Vesper source
/// (<c>5L</c>, <c>3.0</c>, <c>nan</c>, <c>[1; 2; 3]</c>).</summary>
type StructuralPrinter =
    /// <summary>Render <paramref name="value"/> within a column budget of
    /// <paramref name="widthBudget"/> (0 ⇒ never break, the <c>%0A</c> mode) and a node
    /// budget of <paramref name="sizeBudget"/> (F# PrintSize, the <c>%.NA</c> mode).</summary>
