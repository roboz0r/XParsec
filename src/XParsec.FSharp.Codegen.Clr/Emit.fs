...
module Emit =
    ...
    // ---- Structural format (`%A`, P3): synthesised `IStructuralFormattable.Format` ----
    //
    // The synthesised `Format(IFormatSink sink)` is straight-line `callvirt`s on the
    // `sink` arg (`ldarg.1`), mirroring the hand-written `Point`/`Opt` impls in
