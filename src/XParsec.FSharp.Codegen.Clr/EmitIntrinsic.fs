    ...
    let buildILIntrinsic (recur: Recur) (env: EmitEnv) (b: IlBuilder) (e: Frozen.TExpr) : unit =
        ...
        | ExprShape.ILIntrinsic ->
            ...
            | "ldloca" ->
                ...
                match args with
                | [| var |] when
                    TastAccessor.exprKind var = ExprShape.Var
                    && env.Slots.ContainsKey(TastAccessor.exprVarBinding var)
                    ->
                    b.Add(ILInstr.Ldloca env.Slots.[TastAccessor.exprVarBinding var])
