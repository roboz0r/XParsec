// Transliterated from D:\roboz0r\fsharp\src\FSharp.Core\async.fs
...
type TrampolineHolder() =
    ...
    /// Save the exception continuation during propagation of an exception, or prior to raising an exception
    member inline _.OnExceptionRaised econt = trampoline.OnExceptionRaised econt
...
type AsyncActivation<'T>(contents: AsyncActivationContents<'T>) =
    ...
    member ctxt.CallContinuation(result: 'T) = ctxt.cont result
...
module AsyncPrimitives =
    ...
    let CreateForLoopAsync (source: seq<_>) computation =
        CreateUsingAsync
            ...
            (fun ie -> CreateWhileAsync (fun () -> ie.MoveNext()) (CreateDelayAsync(fun () -> computation ie.Current)))
