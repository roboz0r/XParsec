// Transliterated from D:\roboz0r\fsharp\src\FSharp.Core\async.fsi
...
type Async =
    ...
    static member StartAsTask:
        computation: Async<'T> * ?taskCreationOptions: TaskCreationOptions * ?cancellationToken: CancellationToken ->
            Task<'T>
    ...
    static member StartChildAsTask:
        computation: Async<'T> * ?taskCreationOptions: TaskCreationOptions -> Async<Task<'T>>
    ...
    static member AwaitEvent:
        event: IEvent<'Del, 'T> * ?cancelAction: (unit -> unit) -> Async<'T>
            when 'Del: delegate<'T, unit> and 'Del :> Delegate
    ...
    static member internal StartWithContinuationsUsingDispatchInfo:
        computation: Async<'T> *
        continuation: ('T -> unit) *
        exceptionContinuation: (ExceptionDispatchInfo -> unit) *
        cancellationContinuation: (OperationCanceledException -> unit) *
        ?cancellationToken: CancellationToken ->
            unit
    ...
    /// </example>
    static member StartImmediateAsTask: computation: Async<'T> * ?cancellationToken: CancellationToken -> Task<'T>
...
module AsyncPrimitives =
    ...
    /// <returns>A value indicating asynchronous execution.</returns>
    val TryFinally:
        ctxt: AsyncActivation<'T> -> computation: Async<'T> -> finallyFunction: (unit -> unit) -> AsyncReturn
