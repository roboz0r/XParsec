// Transliterated from D:\roboz0r\fsharp\src\FSharp.Core\seq.fs
...
type CachedSeq<'T>(cleanup, res: seq<'T>) =
    ...
    interface IEnumerable with
        member x.GetEnumerator() = (res :> IEnumerable).GetEnumerator()
...
module Seq =
    ...
    let mkUnfoldSeq f x = mkSeq (fun () -> unfold f x)
    ...
    let initInfinite initializer = mkSeq (fun () -> upto None initializer)
    ...
    [<CompiledName("OfList")>]
    let ofList (source: 'T list) = (source :> seq<'T>)
    ...
    let singleton value = mkSeq (fun () -> Singleton value)
