// Transliterated from D:\roboz0r\fsharp\src\FSharp.Core\set.fs
...
module internal SetTree =
    ...
    let inline height (t: SetTree<'T>) = if isEmpty t then 0 else t.Height
    ...
    let rec tryGet (comparer: IComparer<'T>) k (t: SetTree<'T>) =
        if isEmpty t then
            ...
            if t.Height = 1 then
                if c = 0 then ValueSome t.Key else ValueNone
            else
                let tn = asNode t
...
type Set<[<EqualityConditionalOn>] 'T when 'T: comparison>(comparer: IComparer<'T>, tree: SetTree<'T>) =
    ...
    static member Union(sets: seq<Set<'T>>) : Set<'T> = Seq.fold (+) Set<'T>.Empty sets
...
module Set =
    ...
    let partitionWith (partitioner: 'T -> Choice<'T1, 'T2>) (set: Set<'T>) = set.PartitionWith partitioner
