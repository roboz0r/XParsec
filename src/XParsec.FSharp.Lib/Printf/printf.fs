// Transliterated from D:\roboz0r\fsharp\src\FSharp.Core\printf.fs
...
module internal PrintfImpl =
    ...
    type FormatSpecifier =
        ...
        member spec.IsDecimalFormat = spec.TypeChar = 'M'
        ...
        member spec.IsGFormat = spec.IsDecimalFormat || Char.ToLower(spec.TypeChar) = 'g'
        ...
        let parsePrecision (s: string) (i: byref<int>) =
            if s.[i] = '.' then
                if s.[i + 1] = '*' then
                    ...
                    raise (ArgumentException("invalid precision value"))
            else
                NotSpecifiedValue
    ...
    type PrintfEnv<'State, 'Residue, 'Result>(state: 'State) =
        ...
        member env.RunSteps(args: objnull array, argTys: Type array, steps: Step array) =
            ...
            for step in steps do
                ...
                | StepString prefix -> env.WriteSkipEmpty prefix
    ...
    module Padding =
        ...
        let adaptPaddedFormatted
            (spec: FormatSpecifier)
            getFormat
            (basic: string -> objnull -> string)
            (pad: string -> int -> objnull -> string)
            : ValueConverter =
            if spec.IsStarWidth then
                if spec.IsStarPrecision then
                    ...
                    ValueConverter.Make(fun v width -> pad fmt width v)
                ...
                if spec.IsWidthSpecified then
                    // width=val, prec=*
                    ValueConverter.Make(pad fmt spec.Width)
                else
                    // width=X, prec=*
                    ValueConverter.Make(basic fmt)
        ...
        let adaptPadded
            (spec: FormatSpecifier)
            (basic: objnull -> string)
            (pad: int -> objnull -> string)
            : ValueConverter =
            if spec.IsStarWidth then
                // width=*, prec=?
                ValueConverter.Make(fun v width -> pad width v)
            else if spec.IsWidthSpecified then
                // width=val, prec=*
                ValueConverter.Make(pad spec.Width)
            else
                // width=X, prec=*
    ...
    module Basic =
        let leftJustify (f: objnull -> string) padChar =
            fun (w: int) v -> (f v).PadRight(w, padChar)

        let rightJustify (f: objnull -> string) padChar =
            fun (w: int) v -> (f v).PadLeft(w, padChar)
    ...
    module Integer =
        ...
        let eliminateNative (v: objnull) =
            ...
            | :? unativeint as n ->
                if IntPtr.Size = 4 then
                    box (uint32 (n.ToUInt32()))
                else
                    box (uint64 (n.ToUInt64()))
            | _ -> v
        ...
        let rec toString (v: objnull) =
            ...
            | :? nativeint
            | :? unativeint -> toString (eliminateNative v)
        ...
        let rec toFormattedString fmt (v: obj) =
            ...
            | :? nativeint
            | :? unativeint -> toFormattedString fmt (eliminateNative v)
        ...
        let rec toUnsigned (v: objnull) =
            ...
            | :? nativeint
            | :? unativeint -> toUnsigned (eliminateNative v)
            | _ -> v
        ...
        let getValueConverter (spec: FormatSpecifier) : ValueConverter =
            ...
            | 'd'
            | 'i' -> withPadding spec false toString
            | 'u' -> withPadding spec true (toUnsigned >> toString)
            | 'x' -> withPadding spec true (toFormattedString "x")
            | 'X' -> withPadding spec true (toFormattedString "X")
            | 'o' ->
                withPadding
                    spec
    ...
    module FloatAndDecimal =
        ...
        let noJustification (prefixForPositives: string) =
            fun (fmt: string) (v: obj) ->
                GenericNumber.noJustificationCore
        ...
        let leftJustify isGFormat (prefix: string) padChar =
            if isGFormat then
                ...
                fun (fmt: string) (w: int) (v: obj) ->
                    GenericNumber.leftJustifyWithNonGFormat
                        (toFormattedString fmt v)
                        (isNumber v)
                        (GenericNumber.isPositive v)
                        w
    ...
    type ObjectPrinter =
        ...
        static member GenericToStringCore
            (v: 'T, opts: Microsoft.FSharp.Text.StructuredPrintfImpl.FormatOptions, bindingFlags)
            =
            let vty =
                match box v with
                | null -> typeof<'T>
    ...
    let private AllStatics =
        BindingFlags.Public ||| BindingFlags.NonPublic ||| BindingFlags.Static

    let mi_GenericToString =
        typeof<ObjectPrinter>.GetMethod("GenericToString", AllStatics)

    let private getValueConverter (ty: Type) (spec: FormatSpecifier) : ValueConverter =
        match spec.TypeChar with
        | 'b' -> Basic.withPadding spec (unbox >> boolToString)
        | 's' -> Basic.withPadding spec (unbox >> stringToSafeString)
        | 'c' -> Basic.withPadding spec (fun (c: objnull) -> (unbox<char> c).ToString())
        | 'M' -> FloatAndDecimal.withPadding spec (fun _ -> "G") "G" // %M ignores precision
        | 'd'
        | 'i'
        | 'u'
        | 'B'
        | 'o'
        | 'x'
        | 'X' -> Integer.getValueConverter spec
        | 'e'
        | 'E'
        | 'f'
        | 'F'
        | 'g'
        | 'G' -> basicFloatToString spec
        | 'A' ->
            let mi = mi_GenericToString.MakeGenericMethod ty
            mi.Invoke(null, [| box spec |]) |> unbox
        | 'O' -> ObjectPrinter.ObjectToString(spec)
        | 'P' -> ObjectPrinter.InterpolandToString(spec)
        | _ -> raise (ArgumentException(SR.GetString(SR.printfBadFormatSpecifier)))
    ...
    type FormatParser<'Printer, 'State, 'Residue, 'Result>(fmt: string) =
        ...
        let buildCaptureFunc (spec: FormatSpecifier, allSteps, argTys: Type array, retTy, nextInfo) =
            let (next: obj, nextCanCombine: bool, nextArgTys: Type array, nextRetTy, nextNextOpt) =
                nextInfo
            ...
            | _ when spec.TypeChar = 'a' ->
                ...
                let mi =
                    typeof<Specializations<'State, 'Residue, 'Result>>.GetMethod(captureMethName, AllStatics)
        ...
        let parseSpec (i: byref<int>) =
            ...
            let interpHoleDotnetFormat =
                FormatString.parseInterpolatedHoleDotNetFormat typeChar fmt &i
        ...
        // identical results each time, so it is ok.
        let mutable factory =
            Unchecked.defaultof<PrintfFuncFactory<'Printer, 'State, 'Residue, 'Result>>
        ...
        let rec parseAndCreateFuncFactoryAux steps (prefix: string) (funcTy: Type) (i: byref<int>) =
            ...
            if i >= fmt.Length then
                ...
                let allSteps, nextInfo =
                    parseAndCreateFuncFactoryAux (step :: steps) suffix retTy &i
        ...
        let parseAndCreateFunctionFactory () =
            ...
            let allSteps, (factoryObj, _, combinedArgTys, _, _) =
                parseAndCreateFuncFactoryAux [] prefix funcTy &i
            ...
            | [| StepWithArg(prefix1, conv1); StepWithArg(prefix2, conv2); StepString prefix3 |] ->
                ...
                let factoryObj =
                    mi.Invoke(null, [| box prefix1; box conv1; box prefix2; box conv2; box prefix3 |])
        ...
        member this.GetCurriedStringPrinter() =
            ...
            | null ->
                ...
                let initial () =
                    (StringPrintfEnv stringCount |> box :?> PrintfEnv<'State, 'Residue, 'Result>)
    ...
    type Cache<'Printer, 'State, 'Residue, 'Result>() =
        ...
        static let mutable dict: ConcurrentDictionary<string, FormatParser<'Printer, 'State, 'Residue, 'Result>> =
            null
...
module Printf =
    ...
    let gprintf envf (format: Format<'Printer, 'State, 'Residue, 'Result>) =
        ...
        | null ->
            ...
            let initial () =
                (envf cacheItem.BlockCount :> PrintfEnv<_, _, _>)
