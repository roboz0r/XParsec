// Transliterated from D:\roboz0r\fsharp\src\Compiler\Utilities\sformat.fs
                        ...
                        if (path.ContainsKey(x)) then
                            ...
                            let res =
                                ...
                                if ty.IsGenericType && ty.GetGenericTypeDefinition() = typedefof<Lazy<_>> then
                                    ...
                                    | [||] -> None
                                    | NonNull res ->
                                        structuredFormatObjectL
                                            showMode
                                            ty
                                            depthLim
                                            (res[0] :?> StructuredFormatDisplayAttribute)
                                            x
            ...
            if isNull (box txt) || txt.Length <= 1 then
                ...
                let rec buildObjMessageL (txt: string) (layouts: Layout list) =
                    ...
                    if not m.Success then
                        ...
                        | Choice2Of2 e ->
                            Some(wordL (tagText ("<StructuredFormatDisplay exception: " + e.Message + ">")))
                                ...
                                let alternativeObjL =
                                    ...
                                    | _ ->
                                        ...
                                        objL
                                            ...
                                            (alternativeObj, alternativeObj.GetType())
        ...
        and objectValueWithPropertiesL depthLim (ty: Type) (obj: obj) =
            ...
            let propsAndFields =
                ...
                |> Array.filter (fun pi ->
                    ...
                    match
                        Seq.toArray (
                            pi.GetCustomAttributes(typeof<System.Diagnostics.DebuggerBrowsableAttribute>, false)
                        )
                    with
