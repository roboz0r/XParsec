...
- Both surface producers publish a value through `PublishedSurfaceBuilder.addValue`
  (`PublishedSurface.fs:126-147`), handing it a `SourceSpelling` (the declaring scope's
  source path plus the binding's source short name). It writes the compiled name, the
  source-spelled alias beside it, and the module's source path into `ModuleContainers`, so a
  `ModuleSuffix` module's source path is a container — and a path a type resolves
  through — whichever half built the surface (R1).
- Suites at the last landed step: SemanticAnalysis 1472, Clr 1549, Js 666.
...
- A referenced Vesper package is consumed by `PackageProviders.buildProviderSeeded`
  ...
  `PublishedSurface.fs:233-332`); the `.fs` half fully analysed for inline bodies and
...
- The scope-less sources (`Scope = ScopeContents.empty`) are layer 2 only: the BCL metadata
  ...
  (`:648`), the JS stub tables (`JsNativeSymbols.fs:89-97`), and the TS manifest channel.
  No Vesper symbol resolves through them.
...
- **`wholeNameValue`** (`LongIdent.fs:482`) — the whole spelling as a value through
  `ctx.Resolver.TryLookup`, ahead of `typeFirst`. R1 closed the container half: both
  producers register the module's source path in `ModuleContainers`, so `Vesper.List` is a
  container whichever half built the surface. What is left is `[<CompiledName>]`. `scopeOf`
  files every symbol under its KEY's compiled short name (`PublishedSurface.fs:246-249`), the
  alias included, so a scope walk of `Vesper.Set.empty` asks `SetModule` for `empty` and
  misses `Empty`, which the name index hits through the alias. R2 must file the alias under
  its source short name as well — `SourceSpelling.Name`, which `addValue` already holds.
...
2. **R2 — Delete `wholeNameValue`.** First file the source-spelled alias under its source
   short name in `scopeOf`'s `valuesIn`, which is the one Vesper spelling the scope walk
   still misses; the short name is `SourceSpelling.Name` at the `addValue` call, so it needs
   no name surgery. Then the BCL loses nothing either (`MetadataSymbols.TryLookup` is
   `ValueNone`). Expression order becomes `modulePath → typeFirst → folded`. A test that goes
   red identifies a residual feeder: fix the feeder, never reinstate the fallback.
