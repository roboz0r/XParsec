...
/// anonymous unions. The canonicalisation logic and the member order it sorts by
/// now live as `SemType.MkUnion` / `SemType.Compare` / `UnionMembers` in
/// `SemanticInfo.fs` — they must precede the `UnionMembers` payload's private
/// constructor, which sorts in place. This module just keeps the short name every
/// producer already calls.
[<AutoOpen>]
module SemTypeOps =
    ...
    ///   * sort     members by `SemType.Compare` so `string | int ≡ int | string`
