...
module TastWalk =
        ...
        | ValueNone ->
            ...
            | TExpr.App(fn, a, ty, tok) ->
                ...
                if refEq fn' fn && refEq a' a && refEq ty' ty then
                    ...
                    TExpr.App(fn', a', ty', tok)
            | TExpr.Let(m, body, isRec, ty) ->
                let p' = pp m.Pattern
                let v' = pe m.Value
                let body' = pe body
                let ty' = f ty

                if refEq p' m.Pattern && refEq v' m.Value && refEq body' body && refEq ty' ty then
                    e
                else
                    TExpr.Let({ m with Pattern = p'; Value = v' }, body', isRec, ty')
            ...
            | TExpr.StaticFieldSet(k, n, v, ty, tok) ->
                ...
                if refEq v' v && refEq ty' ty then
                    ...
                    TExpr.StaticFieldSet(k, n, v', ty', tok)
            | TExpr.ExternalMember(r, k, n, storage, widths, ty, tok) ->
                ...
                | ValueNone ->
                    ...
                    if refEq ty' ty then
                        ...
                        TExpr.ExternalMember(ValueNone, k, n, storage, widths, ty', tok)
                | ValueSome x ->
                    let x' = pe x
                    let ty' = f ty
                    ...
                    if refEq x' x && refEq ty' ty then
                        ...
                        TExpr.ExternalMember(ValueSome x', k, n, storage, widths, ty', tok)
    ...
    let rec iterExpr (it: Iter) (e: TExpr) : unit =
        if it.VisitExpr it e then
            ...
            | TExpr.App(fn, a, _, _) ->
                walk fn
                walk a
            | TExpr.Let(binding = { Pattern = p; Value = v }; body = body)
    ...
    let declBindings (d: TDeclG<'ty, 'tok, 'id>) : TLetMemberG<'ty, 'tok, 'id> list =
        match d with
        | TDeclG.Let(binding = b) -> [ b ]
        | TDeclG.LetGroup(members = members) -> List.ofSeq members
        | TDeclG.Expression _
        | TDeclG.Type _ -> []

    /// An inline template: a `let inline` decl bound to a simple name, as its bound variable
    /// and binding pattern.
    [<return: Struct>]
    let (|InlineTemplateDecl|_|) (d: TDeclG<'ty, 'tok, 'id>) : struct ('id * TPatG<'ty, 'tok, 'id>) voption =
        match d with
        | TDeclG.Let(
            binding = {
                          Pattern = TPatG.NamedSimple(boundVar = k) as p
                      }
            isInline = true) -> ValueSome(struct (k, p))
        | _ -> ValueNone
    ...
    let declBoundVars (decls: TDeclG<SemType, SyntaxToken, NodeKey> seq) : HashSet<BoundVarKey> =
        ...
        for d in decls do
            for m in declBindings d do
                iterPat it m.Pattern
                ...
                VisitExpr =
                    fun it e ->
                        ...
                        | TExpr.Lambda(p, b, _, _) ->
                            ...
                            false
                        | TExpr.Let(binding = { Pattern = p; Value = v }; body = b) ->
                            iterExpr it v
                            let added = addBoundVars p
                            iterExpr it b
                            removeBoundVars added
