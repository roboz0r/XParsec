...
module FrozenCodecTypes =
    ...
    ///
    /// `Kind.Parse` has no tag and cannot be written. A parse diagnostic belongs to the
    /// UNIT, not to the analysis — it never reaches a frozen file's `Diagnostics` — and its
    /// payload is a parser CST subtree carrying raw char offsets and virtual tokens, the
    /// two things this format exists to keep out. Faulting is the honest answer; a lossy
    /// encoding would let a blob decode to a verdict nobody wrote.
    let private writeKind (w: BinaryWriter) (k: Kind) =
        ...
        | Kind.Message text ->
            ...
            w.Write text
        | Kind.Parse _ ->
            failwith "FrozenCodec: a parse diagnostic belongs to the unit, not to the analysis, and cannot be frozen"

    let private readKind (r: BinaryReader) : Kind =
        match r.ReadByte() with
        | 0uy -> Kind.UndefinedType(r.ReadString())
        | 1uy -> Kind.UnresolvedTyVars(r.ReadInt32())
        ...
        | 9uy ->
            ...
            Kind.MemberNotResolvable(resolver, declaringType, r.ReadString())
        | 10uy ->
            let ty = r.ReadString()
