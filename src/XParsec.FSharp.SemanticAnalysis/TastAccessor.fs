...
module TastAccessor =
    ...
    // A mint takes an `Anchor`, which is what the column holds — `Anchor.nowhere` where the
    // derived node sits at no source position at all. No mint site holds a storage
    // convention, there being none to hold.
