...
module TEnumCases =
    ...
    /// The canonical primitive NAME an integral enum-case literal contributes to
    /// the enum's underlying type. Mirrors `FreezeExpr.constType` / `BuiltinTypes`
    /// — the single `TConstValue`→type-name projection — so these strings are the
    /// established primitive names (all members of `RuntimeNames.numericTypeNames`),
    /// not invented: unsuffixed `Int` is `int` (≡ I32, the default), the authored
    /// wider/unsigned widths name their own CLR underlying type. The four integral
    /// `TConstValue` cases are the only ones a `TEnumLiteral.Int` ever carries (the
    /// elaborator rejects every other constant), so a non-integral value here is a
    /// producer bug, not a user error.
    ...
    /// `true` for an authored *explicit* integral width (`UInt`/`Int64`/`Byte`),
    /// as opposed to the unsuffixed `Int` default — which is width-flexible and
    /// adopts whatever single explicit width is present (else stays `int`). The
    /// uniform-width invariant fires only on a genuine mismatch of *explicit*
    /// widths, so the `Int` default never participates in a conflict.
    let private isExplicitWidth (v: TConstValue) : bool =
        match v with
        | TConstValue.Int _ -> false
        | _ -> true

    /// Derive the enum's underlying primitive type NAME from its resolved case
    /// literals — the single source of truth for "where width lives", DERIVED (not
    /// stored), consistent with `classify`. All-numeric → the uniform integral
    /// width name (the single authored explicit width if any case carries one, else
    /// `int`); all-string → `string`; mixed int+string → `obj` (the heterogeneous
    /// boxed repr); no resolved case → `ValueNone`. The uniform-width *invariant*
    /// (all explicit widths must agree) is NOT enforced here — this derivation is
    /// total and simply takes the first explicit width; `Elaborate` reports a
    /// genuine conflict via `ctx.Error` (see `firstWidthConflict`). Reuses the
    /// canonical primitive names (`numericTypeNames` members / `objAbbrevName`).
    ...
    /// The CLR uniform-width invariant for a numeric enum: a `System.Enum` has
    /// exactly ONE underlying type, so two cases carrying DIFFERENT *explicit*
    /// integral widths (`| A = 1uy | B = 2L`) are illegal. Returns the first
    /// offending case's token paired with the conflicting width names
    /// (first-seen explicit width, then the mismatching one), else `ValueNone`.
    /// Unsuffixed `Int` cases never conflict (they adopt the single explicit width
    /// present); string / mixed enums carry no integral width and never conflict.
