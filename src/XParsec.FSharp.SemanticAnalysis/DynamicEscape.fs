...
// concrete type BEFORE `default ^TResult : dynamic` can fire, which is an unchecked assertion
// that `d.foo` really is that type, so each such site warns. Runs after generalisation.
