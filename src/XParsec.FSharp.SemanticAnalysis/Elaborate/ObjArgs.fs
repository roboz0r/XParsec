...
module internal ElaborateObjArgs =
    ...
    /// key's OWN argSig so one of two same-name overloads cannot be picked in place of the
    /// other. Empty for an external or unresolved member, whose call still emits, just unwrapped.
    ...
    /// field types at the union's type arguments. Empty for a type that is not a union.
    let unionCaseFieldTys (ctx: PassContext) (unionTy: SemType) (caseName: string) : SemType list =
        let noSuchCase () =
            failwithf "Elaborate.unionCaseFieldTys: union %A has no case '%s'" unionTy caseName
