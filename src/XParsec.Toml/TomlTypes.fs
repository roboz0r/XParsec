...
[<Struct>]
type TomlLocalDate = { Year: int; Month: int; Day: int }
...
module TomlTable =
    ...
    let add (key: string) (value: TomlValue) (table: TomlTable) : TomlTable = Map.add key value table

    /// Tries to get a value by key.
    let tryFind (key: string) (table: TomlTable) : TomlValue option = Map.tryFind key table

    /// Gets all keys in the table.
    let keys (table: TomlTable) : string list = table |> Map.toList |> List.map fst

    /// Checks if a key exists.
    let containsKey (key: string) (table: TomlTable) : bool = Map.containsKey key table
...
module TomlDocument =
    ...
    let tryGetPath (path: string) (doc: TomlDocument) : TomlValue option =
        ...
        | firstKey :: rest ->
            ...
            | Some v -> if List.isEmpty rest then Some v else navigate rest v
            | None -> None
    ...
    let tryGetInlineTable (path: string) (doc: TomlDocument) : (string * TomlValue) list option =
        match tryGetPath path doc with
        | Some(TomlValue.InlineTable t) -> t |> Map.toList |> Some
        | _ -> None
