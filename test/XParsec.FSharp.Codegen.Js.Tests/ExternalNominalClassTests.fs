...
let private analyse (input: string) : Diagnostic list =
    ...
    let tast =
        Pipeline.analyseSemFor testCompiling boxProvider (LexedFile.ofText lexed) file
        ...
        [
            ...
            test "GENERIC manifest interface return freezes to FTClass keyed by its arity-suffixed name" {
                ...
                | FTClass(key, args) ->
                    ...
                    Expect.isTrue
                        (match ExternalSymbols.tryMetaTypeAt boxProviderRaw "Wrap`1" 0 with
                         | ValueSome _ -> true
                         | ValueNone -> false)
            ...
            test "NEGATIVE: a type-alias name stays a transparent Abbrev (never FTClass)" {
                ...
                | ValueSome(ExternalTypeShape.Abbrev(typars, target)) ->
