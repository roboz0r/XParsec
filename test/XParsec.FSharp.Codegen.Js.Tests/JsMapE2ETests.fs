...
// `Js.Map<string, int>` against the VENDORED es2015 ref pack, mounted under `Js`. `Map` is
// a Node GLOBAL, so no runtime `.mjs` module is emitted and no import appears.

let private mapContract = contractTs es2015Manifest

let private mapProvider: IExternalSymbolProvider = mapContract.Provider

// Every observed value is a TOP-LEVEL `let`, so library-mode emit exports it
// (`export const …`) and the harness reads it back, with no `printfn`/`%A` over `get`'s
// `undefined | int` result. `n` is read BEFORE the delete, so it observes 2.
let private program =
    String.concat
        "\n"
        [
