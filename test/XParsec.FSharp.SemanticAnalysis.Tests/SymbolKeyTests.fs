...
// Every external-type producer routes through `ExternalSymbols.externalTypeKey`
    ...
    testList
        ...
        [
            test "cons-list: external-ref mint paths equal the local-def canonical key" {
                ...
                let viaOrigin =
                    ExternalSymbols.externalTypeKey
                        {
                ...
                let viaQualified =
                    ExternalSymbols.qualifiedTypeKeyOf (Some "Vesper.List") "Vesper.Collections.List" 1
            ...
            test "ref cell: external-ref mint paths equal the local-def canonical key" {
                ...
                let viaQualified =
                    ExternalSymbols.qualifiedTypeKeyOf (Some "Vesper.Core") "Vesper.Ref" 1
            ...
            test "asm is load-bearing: same (ns, name, arity) but differing home assembly are distinct keys" {
                ...
                let homeB =
                    ExternalSymbols.qualifiedTypeKeyOf (Some "Other.Asm") "Vesper.Collections.List" 1

                let asmless = ExternalSymbols.qualifiedTypeKey "Vesper.Collections.List" 1
                ...
                Expect.equal
                    (ExternalSymbols.qualifiedName homeA)
                    (ExternalSymbols.qualifiedName homeB)
