    ...
    testList
        ...
        [
            ...
            test "`let inline succ x = x + 1 in succ 41` keeps the inline template and outlines its use site" {
                ...
                match edge with
                | TExpr.InlineCall(
                    spec = spec; args = EqList [ TExpr.Const(TConstValue.Integral(IntValue.Int32 41), _, _) ]) ->
                    ...
                    | TExpr.Lambda(TPat.NamedSimple _,
                                   TExpr.InlineCall(
                                       args = EqList [ TExpr.Var _
                                                       TExpr.Const(TConstValue.Integral(IntValue.Int32 1), _, _) ]),
                                   _,
                                   _) -> ()
            ...
            test "freshen leaves a free Var untouched" {
                ...
                let body =
                    TExpr.Let(
                        {
                            Pattern = TPat.NamedSimple(boundKey, tyInt, dummyTok, false)
                            Value = TExpr.Var(freeKey, tyInt, dummyTok)
                            Tok = dummyTok
                        },
            ...
            test "a published body's reference to a NON-inline module sibling is an External carrying its key" {
                ...
                match body with
                | TDecl.Let({ Value = v }, _, _) -> TastWalk.iterExpr collect v
            ...
            test "an inline template referencing a TOP-LEVEL binding IS published" {
                ...
                match body with
                | TDecl.Let({ Value = value }, _, _) -> TastWalk.iterExpr collect value
                | other -> failtestf "unexpected published decl %A" other
