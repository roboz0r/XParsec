...
let private methodTypar (i: int) : FrozenType =
    FTTypar(TyparScope.Member(SymbolKeyOps.qualifiedTypeKeyOf "T" 0), TyparIndex.typeSlot i)

/// The declaring type's typar `i`.
let private declTypar (i: int) : FrozenType =
    FTTypar(TyparScope.Type(SymbolKeyOps.qualifiedTypeKeyOf "Box" 1), TyparIndex.typeSlot i)
    ...
    testList
        ...
        [
            ...
            test "the non-generic overload is preferred over an equally-applicable generic one" {
                ...
                Expect.equal atInt.IsSome true "the int argument resolves"
                Expect.equal atInt.Value.Signature.MethodTyparArity 0<typeSlot> "the non-generic M(int) wins for int"

                let atString = pick candidates [ BuiltinTypes.tyString ]
                Expect.equal atString.IsSome true "the string argument resolves"

                Expect.equal
                    atString.Value.Signature.MethodTyparArity
                    1<typeSlot>
                    "only the generic overload matches string"
            }
