    ...
    testList
        ...
        [
            ...
            test "pair of ints types as int * int" {
                ...
                Expect.equal
                    (declType tast)
                    (TyTuple(Block.ofList [ BuiltinTypes.tyInt; BuiltinTypes.tyInt ]))
                    "p : int * int"
            ...
            test "triple of mixed types" {
                ...
                Expect.equal
                    (declType tast)
                    (TyTuple(Block.ofList [ BuiltinTypes.tyInt; BuiltinTypes.tyBool; BuiltinTypes.tyInt ]))
                    "t : int * bool * int"
            ...
            test "let rec … and …: mutual recursion both see each other" {
                ...
                | EqList [ TDecl.LetGroup(members, components) ] ->
                    ...
                    Expect.equal
                        ...
                        (Block.ofList [ Cycle(Block.ofList [ 0; 1 ]) ])
            ...
            test "float literal types as float" {
                ...
                | TDecl.Let({
                                Value = TExpr.Const(TConstValue.Float v, _, _)
                            },
                            _,
                            _) -> Expect.floatClose Accuracy.medium v 3.14 "value preserved"
            ...
            test "plain string literal types as string" {
                ...
                match tast.Decls.[0] with
                | TDecl.Let({
                                Value = TExpr.Const(TConstValue.String text, _, _)
                            },
                            _,
                            _) -> Expect.equal text "hello" "string body preserved"
                | other -> failtestf "unexpected: %A" other
            }
            ...
            test "lambda with tuple param: `fun (a, b) -> a + b`" {
                ...
                let expected = TyFun(TyTuple(Block.ofList [ intType; intType ]), intType)
            ...
            test "nested tuple pattern: `fun ((a, b), c) -> a + b + c`" {
                ...
                let expected = TyFun(TyTuple(Block.ofList [ TyTuple(Block.ofList [ i; i ]); i ]), i)
            ...
            test "match on tuple with tuple pattern" {
                ...
                let i = BuiltinTypes.tyInt
                let expected = TyFun(TyTuple(Block.ofList [ i; i ]), i)
                Expect.equal (declType tast) expected "f : int * int -> int"
                Expect.isEmpty tast.Diagnostics "no diagnostics"
            }
            ...
            test "inline expansion at declared typars out of appearance order" {
                ...
                let rTy =
                    match List.ofSeq (Block.toList tast.Decls) with
                    | [ _; TDecl.Let(m, _, _) ] -> m.Ty
                    | other -> failtestf "expected two TDecl.Lets, got %A" other

                Expect.equal
                    rTy
                    (TyTuple(Block.ofList [ BuiltinTypes.tyString; BuiltinTypes.tyInt ]))
                    "r : string * int"
            ...
            test "tuple pattern surfaces in TPat shape" {
                ...
                match tast.Decls.[0] with
                | TDecl.Let({
                                Pattern = TPat.Tuple(EqList [ TPat.NamedSimple(_, _, _, _); TPat.NamedSimple(_, _, _, _) ],
                                                     _,
                                                     _)
                            },
                            _,
                ...
                | TDecl.Let({
                                ...
                                Value = TExpr.Lambda(TPat.Tuple(EqList [ TPat.NamedSimple _; TPat.NamedSimple _ ], _, _),
                                                     _,
                                                     _,
                                                     _)
                            },
