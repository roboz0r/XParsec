    ...
    testList
        ...
        [
            test "an annotated value binding's type is declared" {
                ...
                Expect.isTrue (ctx.IsTypeDeclared(lastBoundVarKey tast)) "let x : int = 1 → x declared"
            }

            test "an unannotated value binding's type is inferred" {
                let ctx, tast = analyse "let x = 1"
                Expect.isFalse (ctx.IsTypeDeclared(lastBoundVarKey tast)) "let x = 1 → x inferred"
            }
