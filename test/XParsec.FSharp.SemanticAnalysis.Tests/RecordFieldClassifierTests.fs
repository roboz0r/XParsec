    ...
    testList
        ...
        [
            test "unique exact: single candidate equal to the typed set" {
                ...
                Expect.equal c.ExactCount 1 "exactly one candidate matched"
            ...
            test "ambiguous: two candidates with the same declared set → no exact match" {
                ...
                Expect.equal c.Exact ValueNone "two equal-set candidates ⇒ ambiguous, no exact match"
                Expect.equal c.ExactCount 2 "both matched exactly ⇒ count 2"
            ...
            test "superset: a candidate declaring MORE than typed is partial, not exact" {
                ...
                Expect.equal c.Exact ValueNone "but its set is not EQUAL to typed ⇒ not exact"
                Expect.equal c.ExactCount 0 "no exact match"
            }
            ...
            test "empty candidates → empty verdict" {
                ...
                Expect.equal c.Exact ValueNone "no candidates ⇒ no exact match"
                Expect.equal c.ExactCount 0 "no candidates ⇒ zero exact matches"
            }
            ...
            test "dedup: the same key appears once, first occurrence wins" {
                ...
                Expect.equal c.Exact (ValueSome("R", set [ "X"; "Y" ])) "and the dedup leaves a unique exact match"
                Expect.equal c.ExactCount 1 "the duplicate collapses to one exact match, not two"
            }
        ]
