                ...
                runsSelfHostLines
                    ...
                    (String.concat
                        ...
                        [
                            ...
                            "    let r = render (LGroup(d, false)) 0 false 0 width"
                            "    r.Txt"
                            // `revOnto` reverses the per-frame child accumulator (built by
                            // consing) back into recording order, without a `List.rev` external dep.
                            ...
                            // A union-case frame: its payload count fixes the arity at `EndCase`,
                            // and `ChildAppShaped` records whether the lone payload was itself a
                            // payload-bearing case (so it parenthesises: `Some (Some 1)`).
                            "type SemFrame ="
                            "    val Name: string"
                            ...
                            // Records are unexercised by this sink (only union cases drive it), so
                            // the record ops stay no-op stubs; the case ops are implemented for real.
                            ...
                            "            this.Push(Frame(CaseCollect, 0, false))"
                ...
                runsSelfHostLines
                    ...
                    (String.concat
                        ...
                        [
                            ...
                            // a large `as`-binding type-test match in a member body drops a
                            // binder slot in codegen; test/cast form is equivalent.
                            "let formatPrimitive (value: obj) : string ="
                            ...
                            "    new(kind: FrameKind, nestIndent: int, parens: bool) ="
                            "        { Kind = kind; NestIndent = nestIndent; Parens = parens; Kids = [] }"
                            // A union-case frame (see the PP7b sink): the payload count fixes the
                            // arity at `EndCase`, and `ChildAppShaped` drives single-payload parens.
                            "type SemFrame ="
                            "    val Name: string"
                            "    val mutable Count: int"
                            "    val mutable ChildAppShaped: bool"
