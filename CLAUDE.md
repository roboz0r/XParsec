...
- A stage that discards an intermediate makes consumers re-derive it is wrong.
  Inevitably the two re-derivations disagree, and the disagreement then needs to be fixed by passing
  the intermediate, which should have happened when it was written.
...
- `[<Struct>]` only where minting dominates passing. A value threaded through a provider chain
...
- Scoping is top-down: a name resolves only below its declaration, and an `and`-joined group is
  the one exception. Machinery that resolves a name written later is a bug to remove, not a
...
- F# is order-sensitive, so a `let private` helper must be defined above its first use.
...
- A nested `CLAUDE.md` under a compiler project or `test/` carries that area's rules.
