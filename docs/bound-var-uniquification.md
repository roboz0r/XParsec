# BoundVar uniquification — a shared pass still owed
...
own IIFE arrow (a fresh JS scope per boundVar). The break is confined to `JsStatement.Const` in a
flat statement list. CLR is safe — `EmitBindings` keys a local slot by `NodeKey` and IL locals are
unnamed.

So the pass is owed generally, not just for preamble boundVars. F# uniquifies by source position and
