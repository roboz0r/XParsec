...
| Operator / Function | Description | Example Usage & Result |
| --- | --- | --- |
| `p1 <\|> p2` | Tries `p1`. If it fails, it rewinds the input and tries `p2`. | `pA <\|> pB` on "b" -> `Ok 'b'` |
| `choice ps` | Tries a sequence of parsers `ps` sequentially, rewinding on failure, and returns the result of the first one that succeeds. | `choice [pFail; pB; pA]` on "b" -> `Ok 'b'` |
| `dispatch f` | **Performance Optimization.** Peeks at the next token (O(1)) and uses function `f` to select the correct parser immediately. Does **not** backtrack. Use this for LL(1) grammars. | `dispatch (function 'a' -> pA \| _ -> pB)` |
...
| `p <\|>% x` | Tries `p`. If it fails, it succeeds with the default value `x` (rewinding input if necessary). | `pA <\|>% 'z'` on "b" -> `Ok 'z'` |
