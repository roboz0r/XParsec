...
The parsers consume only the tokens that emerge on the other side: a clean, layout-respecting stream that lets the parser combinators read like a textbook grammar.
...
Each branch dispatches to the necessary transformation: preprocess, skip trivia, synthesise, split, check layout. Nothing in the list is particularly clever in isolation. What's interesting is that they all live behind the same `peek`/`consume` interface.

## Trivia stays in the token array, but not in the parser's view

Post 2 argued for keeping trivia in the lexer's output, every is part of a `PositionedToken`. That's what lets the eight-byte `PositionedToken` get away without a length field.
...
`isTriviaToken` classifies whitespace, line comments, block comments, newlines, and indent tokens as trivia. Anything else falls through. This is one of those "decide once, benefit everywhere" calls. The alternative, having the lexer emit fewer tokens and force tools that want trivia reconstruct it from source spans, would have pushed a lot of fiddly bookkeeping into every consumer that wasn't the parser. Keeping the array whole and filtering at the reading end inverts that.
...
One of the most-used virtual tokens in the language is one most F# users have never written. Every `let` binding in light syntax has an invisible `in` at the end of it. The spec form is:
...
In the legacy verbose syntax these parse differently. There `in` is required so the second becomes `let x = 1 x + 2`: an error saying `1` isn't a function and can't be applied to `x`. In light syntax they're the same code: `in` is synthesised at the boundary between the binding and the body based on layout.
...
let pLetOrUseIn letIndent (reader: Reader<_, ParseState, _, _>) =
    ...
    | Ok t when t.Token = Token.KWIn -> consumePeeked t reader // A real 'in' is accepted and consumed
    | Ok t ->
...
Paren-like contexts short-circuit the check. Inside `(...)` or `[...]` the delimiter itself bounds the scope, so the offside check can't fire, `pLetOrUseIn` emits the virtual `in` unconditionally. Post 6 covers why paren-like contexts sit on the stack as markers rather than offside lines.
...
Nested `let`s in typical F#, `let x = ... let y = ... f x y`, the shape of most F# code people actually write, goes through this three or four times before the expression is complete. Each binding is punctuated with an `in` that shows up in the parse tree with `IsVirtual` set, allowing things like a formatter walking the tree to tell the difference between `in`s the user wrote and `in`s the parser invented.

### The rest of the virtual-token machinery

The same shape repeats for other grammar-mandated tokens. `pDoneVirt` synthesises a virtual `done` to close `for` and `while` bodies.
...
To the AST parsers this virtual-token machinery is transparent. They don't care whether a given token is real or invented, they just know they got a `SyntaxToken` where they expected one, the `Index` discriminator tells them which kind if anyone needs to look later.
...
One more thing happens on every token that survives the earlier branches: the offside check. The context stack, the push/pop discipline, and the *permitted-undentation* rules that let F# flow naturally, fill two posts of their own (posts 6 and 7). What matters here is that the check lives in the same function, in front of every token the parser sees.

The reason is the same as for virtual tokens. The enforcement is uniform as lifting it into every parsing rule would both cost performance and risk forgetting it in one place.

## `#if`, evaluated lazily

F# preprocessor directives operate on tokens: `#if DEBUG && !RELEASE` is a four-token condition. The lexer recognises `#if` and treats tokens on the same line with their lexing rules bu doesn't attempt to evaluate it, allowing lexing to remain purely a function of the input string. The filter evaluates `#if` as it encounters them, and skips tokens from the inactive branches. The driver is `processIfDirective`:
...
The second is the expression parser itself. `IfExpr.parseSlice` (in `Preprocessing.fs`) is a full Pratt parser in about 80 lines of code: `&&`, `||`, `!`, parenthesised sub-expressions, identifier terms. Four binding powers, a half-dozen completion functions. It gets all the same Pratt machinery the real expression parser uses, applied to a ten-token sub-grammar. It's a small demonstration that the library's operator primitives work for F# big expression grammar they were built for, but are simple enough to use in any context requiring operator precedence parsing.
...
That design makes "peek" semantics here different from what many parsers assume. In most systems, peeking is strictly side-effect free. Here it is not. A peek over `// comment\n    foo` advances the reader past the comment and whitespace, then returns `foo`. The reader has been mutated. This mutation is safe because all the skipped tokens are always trivia.

### Peek is allowed to mutate so the grammar can stay cheap

Once `peekNextSyntaxToken` has landed on `foo`, subsequent peeks are O(1). They inspect the same token and return immediately. This common dispatch pattern already only peeks once already:
...
The initial peek pays the cost of filtering; `consumePeeked` advances one token.

But what about peeks made by adjacent parsers? If peek were required to be side-effect free, every peek would have to re-walk trivia from the last committed position. On real F# code, where a large fraction of tokens are whitespace, that would introduce a linear factor into almost every parsing rule.
...
Routes are mostly five items or fewer, so a linear scan beats a dictionary. More importantly, `dispatchNextSyntaxTokenFallback` (unlike `dispatch` in the XParsec core library) operates on the **filtered stream**, so the routing decision is based on a meaningful token.
...
Lexing itself is *not* lazy. The `Tokens` and `LineStarts` arrays are built eagerly in a single sweep before a parser ever runs. That choice pays for itself everywhere: derived token lengths, binary-search diagnostic rendering, and O(1) indexed reads from whichever grammar rule happens to be active.

The obvious next step would be to materialise the following stages too: evaluate all `#if` directives, build a filtered token array, and, hand that array to the parser. XParsec does not, for four reasons:
...
Three passes become one fat helper function. The helper does one thing: hand the next meaningful token to the parser. Each concern earns a few lines in one `match`, not a pass of its own. Everything in `ExpressionParsing.fs`, `PatternParsing.fs`, `TypeParsing.fs`, and `DeclarationParsing.fs` goes through them. None of those files knows that preprocessor directives, or trivia, or offside rules exist.

The architectural payoff is that half the project's hardest features become invisible to the most important part. The parsers beyond can look *almost* like a textbook grammar, because every place where F# breaks the textbook model is enforced once, at the portal.
...
**Lazy filter over eager lexer.** The lexer materialises because the array is indexed and shared. The filter stays lazy because it needs to see state the parser is still mutating, split flags and context frames that couldn't exist at lex time.
...
**Split-on-demand is the price of fuse-and-defer.** Fused operators keep the lexer simple; two booleans and an integer in `ParseState` let the filter hand back the parts the grammar asks for.

**Peek is allowed to mutate, carefully.** Skipping trivia and advancing past filtered tokens keep repeated peeks O(1), and one `reader.Position` assignment still rewinds everything when backtracking.
