...
- **What the corpus found — three categories.** The bugs sorted cleanly into three buckets that nicely explain why hand-written tests had missed them.
  ...
  Plus the long tail: `5dd1a88 Fixing several parsing bugs` (a *lot* of fixes in one commit, all corpus-originated), `17172d4 Parse all string literals as fragments` (every plain string was a single token before this; the corpus surfaced enough downstream consumers wanting fragment-level access — formatters, linters, diagnostics that point inside a string — that the unification finally paid for itself), `9c21b90 Fix parsing multiple secondary constructors`, `51691aa Allow parsing inherit anywhere in type declarations`, `ef56004 Fix parsing of flexible types`, and half a dozen offside fixes referenced in post 7.

  And a thread of lexer fixes that the corpus surfaced once the AST parser was exercising real code: `037467c Fix lexing a block comment with trailing operator chars` (`(* x *)*` and friends), `fbf38d4 Fix applying block comments to FormatPlaceholder and related tokens` (block comments and string-format tokens interacting badly), `d465f45 Fix handling of || in lexer`, `22a8fc7 Fix lexing "(*)"`, `9310e2c Fix lexing colon in operators`. Each one a single-fixture root cause; none of them surfaced in the months of hand-written tests.
