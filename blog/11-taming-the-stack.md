# 11. Taming the stack: tracing, probing, and a second operator rewrite
...
- **Postscript: the pattern repeats, and the second time is cheap.** Commit `7786a60 Reduce stack consumption in AST traversal` (2026-04-18) fixed exactly the same class of bug in `AstTraversal.fs` — a 1,300-line `walkExpr` whose per-call frame was large enough that deep ASTs overflowed even after the parser itself had been fixed. The remedy was the now-familiar one: split the giant pattern-match into one small function per DU arm, each taking the arm's fields as arguments, so the enclosing frame stays small regardless of which branch fires. What took weeks the first time took a single prompt ("the most likely contributor is `walkExpr`; split each arm into its own function") and seven minutes of Claude time. That's the payoff curve for investing in the diagnostics early — the second occurrence of a bug shape is almost free.
