...
Run with BenchmarkDotNet v0.15.8 on .NET 10. `Large` is the F# compiler's `prim-types.fs`: a 7000+ line file that leans heavily on internal and obscure F# language features and is a worst-case stress test for any F# parser. Clearly there's a lot left to do but we'll get to that at the end of the series.
