...
- **The fragmentation pattern.** The same principle drove `17172d4 Parse all string literals as fragments` (covered as a corpus finding in post 10). The AST rule generalises: when you have a structured lexical thing — a string, a list expression, a record literal — don't collapse its internal structure at lex-time just because the *first* consumer doesn't need it. The second consumer usually does. Fragmenting plain strings unified the AST shape with interpolated strings, but the *reason* was tooling: formatters that line up escape sequences, linters that flag `%d` without a matching hole, diagnostics that point inside a string. Captured in the memory note `feedback_ast_preserve_tokens`.
